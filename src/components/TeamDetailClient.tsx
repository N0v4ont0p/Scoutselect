...
export default function TeamDetailClient({ team, events, teamNum, currentSeason }: Props) {
  const { t } = useI18n();
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <h2 className="text-lg font-bold mb-4">
        {t.teamDetail.eventsTitle.replace("{season}", seasonName(currentSeason))}
      </h2>

      {events.length === 0 ? (
        <div className="glass rounded-2xl p-8 text-center" style={{ border: "1px solid var(--border)" }}>
          <Calendar className="w-8 h-8 mx-auto mb-3 opacity-30" />
          <p style={{ color: "var(--text-muted)" }}>{t.teamDetail.noEvents}</p>
        </div>
      ) : (
